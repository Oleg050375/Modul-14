...
cursor.execute('SELECT COUNT(*) FROM Users')  # запрос на подсчёт кол-ва записей в БД
us_cnt = cursor.fetchone()[0]  # сохранение результата запроса в переменную

cursor.execute('SELECT SUM(balance) FROM Users')  # запрос на суммирование всех балансов
bal_sum = cursor.fetchone()[0]  # сохранение результата запроса в переменную

print(bal_sum / us_cnt)  # вычисление и печать среднего значения баланса
