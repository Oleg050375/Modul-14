...
from crud_fuctions_1 import *
import asyncio

""" Наполнение БД продуктов
...
class RegistrationStates(StatesGroup):
    ...
    email = State()  # почта
    age = State()  # возраст
    balance = State()  # баланс на счету

@dp.message_handler(text=['Регистрация'])  # хэндлер обработки запроса на регистрацию
async def sign_up(message):
    await message.answer('Введите имя пользователя (только латинский алфавит):')  # вывод клавиатуры
    await RegistrationStates.username.set()  # ожидание ввода имени и запись его в соответствующий атрибут
...
@dp.message_handler(state=RegistrationStates.username)  # хэндлер обработки, реагирующий на изменение атрибута имени
async def set_username(message, state):
    if is_included(message.text):
        await message.answer('Пользователь существует, введите другое имя:')
        await RegistrationStates.username.set()  # ожидание ввода имени
    else:
        await state.update_data(username=message.text)  # сохранение имени пользователя в локальной БД машины состояний
        await message.answer('Введите свой email:')
        await RegistrationStates.email.set()  # переход в состояние ожидания ввода почты


@dp.message_handler(state=RegistrationStates.email)  # хэндлер обработки, реагирующий на изменение атрибута почты
async def set_email(message, state):
    await state.update_data(email=message.text)  # сохранение почты пользователя в локальной БД машины состояний
    await message.answer('Введите свой возраст:')
    await RegistrationStates.age.set()  # переход в состояние ожидания ввода возраста


@dp.message_handler(state=RegistrationStates.age)  # хэндлер обработки, реагирующий на изменение атрибута возраста
async def set_age(message, state):
    await state.update_data(age=message.text)  # сохранение возраста пользователя в локальной БД машины состояний
    data = await state.get_data()  # считывание всех атрибутов машины состояний
